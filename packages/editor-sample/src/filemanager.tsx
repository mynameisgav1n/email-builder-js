...
function FileExplorerPage() {
  ...
  // Fetch current username from /api/user.php
  useEffect(() => {
  ...
  // Fetch file list (after username loads)
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Stack direction="row" spacing={2} alignItems="center" mb={2}>
        ...
        <Button variant="outlined" startIcon={<AddIcon />} onClick={() => setNewFolderDialog(true)}>
          New Folder
        </Button>
      </Stack>
          ...
          {items.map(it => (
            <Grid item xs={3} key={it.name}>
              <Card sx={{ position:'relative', p:1 }}>
                <IconButton size="small" sx={{ position:'absolute', top:4, left:4 }}
                  onClick={()=>{ setRenameItem(it); setRenameValue(it.name); }}>
                ...
                <CardContent sx={{ cursor:'pointer', textAlign:'center', pt:2 }}
                  onClick={()=>{
                    if(it.type==='folder'){
      ...
      <Dialog open={!!renameItem} onClose={()=>setRenameItem(null)} fullWidth maxWidth="xs">
        ...
        <DialogActions>
          ...
          <Button disabled={!renameValue.trim()||renameValue===renameItem?.name}
            onClick={doRename}>Save</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={newFolderDialog} onClose={()=>setNewFolderDialog(false)} fullWidth maxWidth="xs">
        ...
        <DialogContent>
          <TextField autoFocus fullWidth size="small" label="Folder Name"
            value={newFolderName} onChange={e=>setNewFolderName(e.target.value)}/>
        </DialogContent>
        <DialogActions>
          <Button onClick={()=>setNewFolderDialog(false)}>Cancel</Button>
          <Button onClick={doCreateFolder} disabled={!newFolderName.trim()}>Create</Button>
        </DialogActions>
      </Dialog>

      {/* Lightbox: fullScreen + transparent */}
      <Dialog
        open={lightboxOpen}
